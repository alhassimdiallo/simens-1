...
class ConsultationController extends AbstractActionController {
	...
	public function consultationMedecinAction() {
		...
		if ($this->params ()->fromRoute ( 'terminer' ) == 'save') {
			...
			// Donnï¿½es COMMUNES A TOUTES LES INTERFACES
			...
			/**
			 * *******************FIN FIN Examen complï¿½mentaire *****************
			 */
			/**
			 * *******************FIN FIN Examen complï¿½mentaire *****************
			 */
			...
			$tab [0] = $id_ordonnance; // rï¿½cupï¿½rer l'id de l'ordonnance et l'ajouter dans le tableau
			$medicaments = new Consultation_Model_Managers_OrdonConsommable ();
			$medicaments->addOrdonConsom ( $tab ); // on ajoute les mï¿½dicaments dans l'ordonnance
			...
			// Crï¿½er le traitement chirurgical
			...
			// crï¿½er le Rendez vous et inserer le RV
			$infos_rv = array (
			...
			// On ajoute la demande dans la table Demande_Examen s'il y'a au moins un examen demandï¿½
			...
			// On ajoute la liste des examens demandï¿½es dans la table Demande_Liste_Examen
	...
	public function majComplementConsultation() {
		...
		// //instancier les donnï¿½es du rendez-vous mï¿½dical
		// $rv = new Consultation_Model_Managers_RvPatientCons();
		// $rendez_vous = $rv->getRendezVous($id);
		// $this->view->verifieRV = $rendez_vous;

		// //instancier les donnï¿½es du transfert vers un autre service
		...
		// $ordonnance = new Consultation_Model_Managers_Ordonnance ();
		// $idOrdonnance = $ordonnance->getIdOrdonnance($id); //on rï¿½cupï¿½re l'$id de l'ordonnance
		// $this->view->idOrdon = $idOrdonnance;

		// //Instancier la liste des demandes d'examens effectuï¿½es
		...
		// //rï¿½cupï¿½ration de la liste des services de l'hopital concernï¿½
		...
		// //Les donnï¿½es des constantes prises
		// 'id_cons' => $consult ['ID_CONS'],
		// 'id_medecin' => $consult ['ID_PERSONNE'],
		// 'id_patient' => $consult ['PAT_ID_PERSONNE'],
		// 'motif_admission' => $consult ['MOTIF_ADMISSION'],
		...
		// //Les donnï¿½es de l'analyse biologique
		...
		// //Les donnï¿½es de l'examen morphologique
		...
		// //Les donnï¿½es du rendez vous mï¿½dical
		// 'motif_rv' => $rendez_vous['NOTE'],
		// 'date_rv' => $date_RV,
		// 'heure_rv' => $rendez_vous['heure'],

		// //Les donnï¿½es du transfert du patient
